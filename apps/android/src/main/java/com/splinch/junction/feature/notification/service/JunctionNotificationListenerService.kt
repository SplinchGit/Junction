package com.splinch.junction.feature.notification.service
...
import com.splinch.junction.feature.notification.NotificationTapStore
...
class JunctionNotificationListenerService : NotificationListenerService() {
    ...
    override fun onListenerConnected() {
        ...
        activeNotifications?.forEach { sbn ->
            if (!shouldIgnoreNotificationPackage(sbn.packageName)) {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        if (shouldIgnoreNotificationPackage(sbn.packageName)) return
        liveNotifications[sbn.key] = sbn
        enqueueNotification(sbn)
    }
    ...
    private fun enqueueNotification(sbn: StatusBarNotification) {
        ...
        if (packageName == applicationContext.packageName || shouldIgnoreNotificationPackage(packageName)) return
    ...
    companion object {
        ...
        /** Notifications Junction deliberately does not turn into feed items. */
        private val ignoredNotificationPackages = setOf(
            // Status-bar, charging and other OS state is already visible to the owner.
            "com.android.systemui",
            "com.samsung.android.sm",
            "com.samsung.android.lool",
            "com.google.android.as",
            // Browser tab/download/background noise is not an actionable Junction inbox item.
            "com.android.chrome",
            "org.mozilla.firefox",
        ...
        private fun shouldIgnoreNotificationPackage(packageName: String): Boolean =
            packageName in ignoredNotificationPackages
