...
class LocalBrainRelay {
  ...
  async run(state,item){try{await this.update(item.document,state.session,{status:"processing"},item.document.updateTime);const id=item.data.id||item.document.name.split("/").pop();const text=crypt(state.key,`JBP1|${state.brainId}|${id}|request`,Buffer.from(item.data.ciphertext,"base64url"),Buffer.from(item.data.nonce,"base64url"),false);const payload=JSON.parse(text);const upstream=await this.fetch(`${this.ollamaUrl}/api/chat`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({model:payload.model,messages:payload.messages,stream:false,think:true,options:{num_predict:1024}})});const result=await upstream.json(),answer=String(result?.message?.content||"").trim();if(!upstream.ok||!answer)throw new Error(result?.error||"Local model returned an empty response.");const response=crypt(state.key,`JBP1|${state.brainId}|${id}|response`,Buffer.from(answer),null,true),thought=String(result?.message?.thinking||"").trim(),thinking=thought?crypt(state.key,`JBP1|${state.brainId}|${id}|thinking`,Buffer.from(thought),null,true):null,tps=Number(result.eval_count)&&Number(result.eval_duration)?(Number(result.eval_count)/(Number(result.eval_duration)/1e9)).toFixed(1):"";await this.update(item.document,state.session,{status:"done",responseCiphertext:response.ciphertext,responseNonce:response.nonce,thinkingCiphertext:thinking?.ciphertext||"",thinkingNonce:thinking?.nonce||"",tokensPerSecond:tps},null);}catch(error){try{await this.update(item.document,state.session,{status:"error",error:String(error.message||error).slice(0,300)},null);}catch{}}}
}
