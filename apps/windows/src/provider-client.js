...
function boundedMessages(messages, memories, context){
...
async function sendChat({config,key,messages,memories,context}){
  const definition=PROVIDERS[config.id]; if(!definition) throw new Error("Configure an AI provider first."); if(!key) throw new Error("This PC does not have an API key for the selected provider.");
  ...
  const prompt=boundedMessages(messages,memories,context);
  ...
  const response=await fetch(`${base}/chat/completions`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${key}`},body:JSON.stringify({model,messages:prompt,max_tokens:1200,stream:false})}); const data=await response.json(); if(!response.ok) throw new Error(data.error?.message||`Provider request failed (${response.status}).`); return {content:data.choices?.[0]?.message?.content||"",usage:data.usage||null,model:data.model||model};
}
